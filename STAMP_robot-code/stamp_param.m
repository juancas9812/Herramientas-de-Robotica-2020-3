...
function [STAMP, PARAM] = stamp_param()
...
%sigma=0 (articulaci?n Rotacional), Sigma=1 (articulaci?n prism?tica)
 ...
 %LINK     theta      d        a         alpha      sigma
  L1=Link([  pi/2        0.2      0         -pi/2      0  ],'standard');
  L2=Link([  -pi/2    0        0.4       0          0  ],'standard');
  L3=Link([  pi/2    0        0.4       0          0  ],'standard');
  L4=Link([  3*pi/4        0        0         -pi/2      0  ],'standard');
  L5=Link([  0        0        0         pi/2      0  ],'standard');

 %Ensamble del robot PUMA usando la funcion robot() del toolbox de robotica
 %de Peter Corke
 STAMP = SerialLink([L1 L2 L3 L4 L5]);
 STAMP.name='STAMP';
...
%q = [0 0 0 0 0];        %postura de cada articulaci?n [rad]
% %[q,qd,qdd] = joint_traj_puma560();
% 
% STAMP.plot3d(q);
%STAMP.teach(q);         %GUI para manejar cada articulacion              
